...
HeatFeatures <- function(obj,
  ...
  colnames(heatmat) <- gsub('-','_',colnames(heatmat))  # 根据自己的需要修改
...
GetProgram <- function(features  = FeatureMatrix,
  ...
  c5gobp <- read.gmt('/GPUFS/sysu_zhaom_1/QG/datasets/GSEA/c5.go.bp.v2023.1.Hs.symbols.gmt')
  if (geneset == "GO") c5 = subset(c5gobp, term %like% "GO")
  reactome <- read.gmt('/GPUFS/sysu_zhaom_1/QG/datasets/GSEA/c2.cp.reactome.v2023.1.Hs.symbols.gmt')
  if (geneset == "REACTOME") c5 = subset(reactome, term %like% "REACTOME")
